...
export default defineComponent({
    ...
    methods: {
        ...
        async confirmDelete(id, index) {
            this.isLoading = true;
            await utils.deleteIndexDialog(route('client.destroy', id), this.clients.data, index);
    ...
    <app-layout :title="title">
        ...
        <template #toolbar>
            <div class="d-flex align-items-center gap-2 gap-lg-3"
                v-if="clients.data.length < allowed_clients?.data?.allowed_clients">
        ...
        <Head title="Clients" />
        <div v-if="clients.data.length > allowed_clients?.data?.allowed_clients">
            <PaymentDueWarning :data="allowed_clients?.data" />
        </div>
        <div class="card">
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5">
                        ...
                        <tbody class="fw-semibold text-gray-500" v-if="clients?.data?.length > 0">
                            <tr v-for="(client, index) in clients.data" :key="index">
                                ...
                                <td class="min-w-150px">{{ client?.linkedin_profile }}</td>
                                <td>{{ client?.skype_profile }}</td>
                                <td>{{ client?.tax_number }}</td>
                                <td>
                                    <div class="form-switch form-check-solid d-block form-check-custom form-check-success">
                                        <input class="form-check-input h-20px w-30px" type="checkbox"
                                            @input="updateStatus(client.id, $event.target.checked)"
                                            :checked="client?.status == 1 ? true : false" />
                                    </div>
                                </td>
                                <td class="min-w-150px">{{ client?.created_at }}</td>
                                <td class="min-w-150px">
                                    ...
                                    <div class="text-left dropdown-menu menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                        ...
                                        <div class="menu-item px-3">
                                            <span @click="confirmDelete(client.id, index)" class="menu-link"><i
                        ...
                        <tbody class="fw-semibold text-gray-600" v-else>
                            <tr class="text-gray-600 fw-bold fs-7 align-middle text-uppercase h-100px">
                                <td colspan="7" class="text-center h-full">
                                    <NoRecordMessage />
                                </td>
                            </tr>
                        </tbody>
            ...
            <div class="card-footer" v-if="clients?.data?.length > 0">
