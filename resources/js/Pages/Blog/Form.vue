...
export default defineComponent({
    ...
    methods: {
        submit() {
            ...
            if (!this.v$.form.$invalid) {
                this.form.transform((data) => ({
                    ...
                    .post(route().current() == 'blog.create' ? this.route("blog.store") : this.route('blog.update', this.form.id), {
                        onSuccess: (data) => {
        ...
        async uploadImage(e) {
            ...
            const data = await utils.imageUpload(route("image.store", 'blog'), e, this.image?.data?.entity_id);
            if (data.response) {
...
<template>
    ...
    <AppLayout :title="isEdit ? 'Edit Blog' : `Add New Blog`">
        <template #breadcrumb>
            <li class="breadcrumb-item">
                <span class="bullet bg-gray-400 w-5px h-2px"></span>
            </li>
            <li class="breadcrumb-item">
                <Link href="/blogs" class="text-muted text-hover-primary">{{ title }}</Link>
            </li>
