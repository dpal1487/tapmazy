...
export default defineComponent({
    props: ["projects", "status", "allowed_projects" ,"clients"],
    data() {
        return {
            form: {},
            action: "project.page",
...
<template>
    ...
    <app-layout :title="title">
        ...
        <template #toolbar>
            ...
            v-if="projects.data.length < allowed_projects?.data?.allowed_projects">
                <Link href="/project/create" class="btn btn-sm fw-bold btn-primary">
                <i class="bi bi-plus-circle"></i>Add New Project</Link>
            </div>
        </template>
        <div v-if="projects.data.length > allowed_projects?.data?.allowed_projects">
            <PaymentDueWarning :data="allowed_projects?.data" />
        </div>
